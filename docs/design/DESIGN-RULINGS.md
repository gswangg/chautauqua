...
- **Reading an artifact is not a grid job, and neither is knowing one exists.** The per-cell *File* and *Response* links were a third path to two things already routed — one speaker (row → detail) and one task across everyone (column). Removing them, an interim design put a muted ◆ in cells with an attachment; that was also wrong, for a sharper reason: **"is something attached?" is not a question anyone asks while scanning, because it is not actionable** — the task is done either way. A glyph needing a legend to answer a question nobody asked is worse than nothing.

  What *is* actionable at a glance is **per-column progress**: which task is lagging across the roster. So each column head now carries name · due · **"7 of 12 done"** — in ink when short of the roster, muted when complete — and the name is the link into the task view. The head becomes both the aggregate and the entry point, so the grid adds no controls and answers a question it previously could not. Cells carry status and nothing else.
- **Status tokens pair by density, not by device.** The dense variant (no padding, ~20px) is for a matrix cell that must fit a 178px column twelve times over; the roomy variant (`min-height:44px`, horizontal padding) is for anywhere a row can carry a real target — phone cards, the speaker detail, the task views. **Colour and weight are identical in both; only geometry differs**, so the same field never changes vocabulary between screens. Both must be edited together: changing one alone is what put a bordered Pending box back on three phone frames after the desktop one had gone bare.
- **A per-selection due date is not worth its cost.** A due date lives on the `task`, not the assignment (`task_assignment` carries only `pending | complete`), so moving it for three of twelve speakers needs a per-assignment override column — and the moment that exists, a column head can no longer state one due date, "overdue" becomes per-speaker, and every reminder must quote that speaker's own date rather than the task's. The two outcomes that get most of the value without forking the model: **move the date for the whole task** (task-view header) and **mark it not needed** for the individuals it does not apply to. The bulk bar therefore offers *Remind these* and *Not needed for these*, and says where the date is changed.
- **Weight goes to the exception, never to the resting state.** Complete is the majority of cells in the speakers grid — 30-odd of 42 in the seeded event — so rendering it filled olive turned the page into a wall of the one status nobody needs to act on, and buried the three that matter. Inverted: **overdue is filled ink** (the only strong mark on the page), **pending is outlined**, **complete is muted text with no fill, border or radius**. Same for participation: *Confirmed* is plain muted text; *Declined* takes the filled treatment. The legend reads "Overdue is filled · pending outlined · complete recedes". Knock-ons in the same pass: per-cell **Response** and **File** links drop to muted 600 so they stop out-shouting the status beside them, and the six per-column **Edit** links collapse into one **Edit tasks** beside the Speaker column head.

  The general rule, since this recurs: **count the cells before choosing the weight.** A status vocabulary designed on a sample row looks balanced; on a real grid the common state dominates whatever treatment it was given. Emphasis is a budget, and the resting state should spend none of it.
...
- **The wordmark needs an optical nudge, and it depends on the alignment mode.** Familjen Grotesk's font box is asymmetric — at 22px it reports ascent 23 / descent 5, so 1.27em of ink inside a 1em box. With `line-height:1` the −3px half-leading pushes the glyphs down inside their own box, so a box-centred flex row leaves the ink ~3px low against Figtree, which is near-symmetric. No line-height value reconciles them.
  - **In an `align-items:center` header** (every admin and public header): the wordmark carries `line-height:1` plus `position:relative; top:-3px` at 22px, `-2.5px` at 20px, `-2px` at 17px.
  - **In an `align-items:baseline` run** (the Docs header, where the wordmark pairs with a "Docs" label): the nudge goes on **the run**, not the wordmark inside it. The run is itself a flex item in a centred header, so its box centres while the ink sits 3px low inside — baseline alignment reconciles the two children with each other, never the pair with the bar. Nudging the inner span instead breaks the shared baseline, since `top` is a post-layout offset baseline alignment cannot absorb.
  - So the rule is one sentence: **nudge the outermost element that carries the wordmark's ink**, −3px at 22px. A find-and-replace on the wordmark string alone will hit the wrong element in baseline runs.
- **Focus**: 2px olive outline, 2px offset, on every interactive element. Never removed, never replaced by the hover treatment.
- **Disabled**: `#8E8A7A` on a `#DDD8C8` border. It has exactly **two** legal uses: an **inert control** (a pager arrow with no page to go to, a Remove at its lower bound) and a **drag handle** (`⋮⋮`, which is chrome, not content). Nothing else — never placeholder text, never de-emphasis, never a "finished" or "empty" value. It fails the 4.5 floor at 3.06:1 on paper, so anything carrying meaning in it is unreadable by someone who needs the contrast. **De-emphasis is done with weight, not lightness**: drop 800 → 600 and keep `#565A4B` (6.28:1), which is what the resting states use. This token escaped into three separate designs in consecutive rounds — a ◆ artifact mark, a finished-column count, and empty field values — each time as "the quiet version of something", which is the exact reasoning to distrust.
- **The 44px floor, and how it gets evaded.** Every interactive element on a phone is `min-height:44px` plus centred flex **plus horizontal padding** — padding alone does not reach the floor, and without padding the hit box is only as wide as the text. Two failure modes to watch, both of which happened here: a **shared token** used verbatim in a dense desktop grid and a 390 card cannot satisfy both, so pair them (`DONE`/`DONE_M`, as the file already does for `hintStyle`/`hintStyleM`); and **never author a `min-height` below 44px on a phone token** — a hardcoded `32px` on the participation menu survived a bundle-wide sweep because that sweep matched the literal strings `36px`/`38px`/`40px`.
