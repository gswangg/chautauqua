...
| `chq-speakers-name-link` | ROOMY | app/src/pages/speakers/speakers.css | **Unsatisfied** (off-limits) | Name cell's Link is explicitly the row's only navigation target per the code's own DEC-930 comment (`OnboardingGrid.tsx:1136`: `DEC-930: the grid's name cell is the link`) — the `<tr>` itself is not clickable, so the link must carry its own floor; no phone rule found. |
