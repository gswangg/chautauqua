...
3. **scripts/perf-seed.ts's task_assignment seeding had zero overlap with
   ...
   Merge note: task-w16-d landed the same fix on `main` concurrently, using
   the gate `contactsPerTaskCount <= acceptedContactIds.length ?
   acceptedContactIds : contactIds`. The two forms select the identical
   contact ids, in the identical order, on both shipped profiles (default
   -> `contactIds`; aie -> the first 80 `acceptedContactIds`), so the merge
   kept main's already-validated form and dropped this branch's duplicate.
