...
export const DEC_367 = "Amendment (wave 94): A cascade shadow is LIVE only when no later declaration that also applies at 390 restores the phone value; the scan counts live shadows and its ratchet goes to zero-tolerance";
...
export const DEC_383 = "Amendment (wave 101): the control-font register is a PER-RULE vocabulary contract: the global reset does not discharge it, and a control-class rule that sets a type register declares font-family: inherit beside it";
export const DEC_384 = "The wave-3 render-sweep gate is log-only; its open items become wave-4 tasks, never in-lane patches";
export const DEC_385 = "Amendment (wave 102): consolidating a sheet's phone blocks preserves SOURCE ORDER — later blocks' rules are appended after earlier ones, and a rule whose meaning changes under the merge is a finding, never a silent edit";
...
export const DEC_393 = "Amendment (wave 95): the 44px sweep's population stopped at app/src, and its token reader only speaks React — the SSR half is built, and the wave-94 carrier citation is corrected";
