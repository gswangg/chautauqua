...
describe("DEC-338 (wave-31 amendment): reviewer queue issues concurrent Promise.all waves", () => {
  it("has more than one repo statement simultaneously in-flight (behavioural, not a source grep)", async () => {
    ...
    // Wave 1 alone holds 4 concurrent statements for an organizer
    // (resolveReviewerSubmissions's first statement, listSubmissionIdsRatedBy,
    // listEvaluationScoresForReviewer, listRecusalsForReviewer); wave 2 holds
    // countEvaluationsBySubmission (id-scoped since the DEC-829 wave-29
    // amendment, so it depends on wave 1's `scoped`) alongside the
    // format/audience-level batches -- a fully serial handler could never
    // exceed 1.
