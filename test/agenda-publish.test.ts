...
// state — publish just returns { published: <scheduled count> } and lets
// the global bumpPublicVersionMiddleware (src/server/pubcache.ts) do the
// actual purge, matching the established pattern in
...
describe("POST /events/:eventId/agenda/publish (AIA-07, DEC-155)", () => {
  ...
  it("200s with { published: N } counting placed sessions, and purges the public cache", async () => {
