...
// already was -- listAudienceLevelsBySubmission is the exact twin of
...
import { listAudienceLevelsBySubmission } from "../src/server/repo/review/submissions";
...
describe("listAudienceLevelsBySubmission (DEC-857)", () => {
  it("returns [] map for an empty id list without touching the db", async () => {
    ...
    const map = await listAudienceLevelsBySubmission(db, []);
  ...
  it("issues ONE query per chunkIds batch, filters to AUDIENCE_LEVEL_FIELD_ID, and returns the label verbatim; absent submissions map to null", async () => {
    ...
    const map = await listAudienceLevelsBySubmission(db, ids);
  ...
  it("filters on AUDIENCE_LEVEL_FIELD_ID (field_audience_level), not the session-format field", async () => {
    ...
    await listAudienceLevelsBySubmission(db, ["sub-1"]);
...
vi.mock("../src/server/repo/review", async () => {
  ...
  return {
    ...
    listAudienceLevelsBySubmission: vi.fn(async (_db: unknown, ids: string[]) =>
