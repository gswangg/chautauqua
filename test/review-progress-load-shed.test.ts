// DEC-351 regression coverage: /plans/:id/progress and /plans/:id/remind must
// stop loading every evaluation row of the round -- they source completed
// reviewer+submission pairs from repo.listCompletedPairsForPlan (reviewerId
// + submissionId only) instead of repo.listEvaluationsForPlan (full scored
// rows, which /results genuinely needs per DEC-345). This test asserts the
...
const listCompletedPairsForPlan = vi.fn(async (_db: unknown, planId: string, round: number) =>
  planId === plan.id && round === plan.currentRound ? COMPLETED_PAIRS : [],
);
...
describe("DEC-351: /progress and /remind load-shedding is wire-identical", () => {
  it("GET /progress returns the pre-fix wire shape without calling listEvaluationsForPlan", async () => {
    ...
    expect(listCompletedPairsForPlan).toHaveBeenCalledWith(expect.anything(), plan.id, plan.currentRound);
  });
