...
describe("dialog contract (DEC-631)", () => {
  ...
  // role="dialog" JSX. PipelineBoard's enroll dialog (DEC-859) is the
  // latest to fold in, joining ViewTabs/Submissions/Speakers/etc. That
  // collapses many previously-duplicated per-file occurrences into ONE
  // (ModalFrame's own tag), so the floor here guards against the glob
