...
// full CONTENT_STATUSES vocabulary), never through the narrowed guard.
...
describe("BARE_CONTENT_STATUS_WRITES / isValidBareContentStatusWrite", () => {
  it("only pending and approved are valid bare writes", () => {
    expect(BARE_CONTENT_STATUS_WRITES).toEqual(["pending", "approved"]);
    expect(isValidBareContentStatusWrite("pending")).toBe(true);
    expect(isValidBareContentStatusWrite("approved")).toBe(true);
    expect(isValidBareContentStatusWrite("changes_requested")).toBe(false);
    expect(isValidBareContentStatusWrite("bogus")).toBe(false);
  });
});
...
describe("POST /api/v1/submissions/:id/content-status (single route)", () => {
  ...
  it("400s on an unrelated invalid value listing only pending/approved", async () => {
    ...
    expect(body.error.message).toBe("contentStatus must be one of pending, approved");
...
// vocabulary), never through the narrowed bare-write guard — otherwise
...
describe("content-notes.ts stays the single changes_requested writer", () => {
  it("writes via updateContentStatus, not the narrowed bare-write guard", () => {
    expect(contentNotesSource).toMatch(/updateContentStatus\(c\.var\.db, submissionId, "changes_requested"\)/);
    expect(contentNotesSource).not.toMatch(/isValidBareContentStatusWrite/);
  });
});
